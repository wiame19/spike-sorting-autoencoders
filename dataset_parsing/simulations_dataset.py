...
def load_dictionaries(simNr):
    ...
    # simulation_dictionary = loadmat(SYNTHETIC_DATA_FOLDER_PATH + '/simulation' + str(simNr) + '.mat')
...
def get_spikes_waveforms(simNr):
    simulation_dictionary, ground_truth_dictionary = load_dictionaries(simNr)
    formes = ground_truth_dictionary['su_waveforms'][0][simNr - 1][:, :]
    return formes
...
def get_dataset_simulation(simNr, spike_length=79, align_to_peak=True, normalize_spike=False, scale_spike=False):
    ...
    spikes = spike_preprocess(data, start, spike_length, align_to_peak, normalize_spike, scale_spike) #spikes alignés
    # spikes = spike_extract(data, start, spike_length) #spikes non alignés

    # print(len(spikes),",",len(spikes[0])) #dimension spikes
    # print(len(labels))
    return spikes, labels, data


# def get_dataset_all_simulations(spike_length=79, align_to_peak=True, normalize_spike=False, scale_spike=False):
#     """
#     Load the dataset
#     :param spike_length: integer - length of spikes in number of samples
#     :param align_to_peak: integer - aligns each spike to it's maximum value
...
#     nb_spikes=0
#
#     class_map_new = np.load("class_map_new.npy", allow_pickle=True)
#     class_map_new = class_map_new.item()
#
#     for i in range(1,25):
#         simulation_dictionary, ground_truth_dictionary = load_dictionaries(i)
#
#         labels = ground_truth_dictionary['spike_classes'][0][i - 1][0, :]
#         start = ground_truth_dictionary['spike_first_sample'][0][i - 1][0, :]
#         data = simulation_dictionary['data'][0, :]
#
#         # each spike will contain the first 79 points from the data after it has started
#         spikes = spike_preprocess(data, start, spike_length, align_to_peak, normalize_spike, scale_spike) #spikes alignés
#         # spikes = spike_extract(data, start, spike_length) #spikes non aligné
#
...
#     spikes_all = np.concatenate(spikes_all)
#     spikes_all = np.reshape(spikes_all, (nb_spikes, 79))
#
#     return spikes_all, labels_all, data_all
...
def get_dataset_all_simulations(spike_length=79, align_to_peak=True, normalize_spike=False, scale_spike=False):
    ...
    for i in range(1,25):
        ...
        for j in range(len(labels_new)):
            for indice, mappings in class_map_new.items():
                for num_simu, clas in mappings.items():
                    if num_simu==i and labels[j]==clas:
                        labels_new[j]=int(indice)
